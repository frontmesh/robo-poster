...
init flags =
    let
        ...
        loadCmd =
            if flags.token /= Nothing then
                Api.getPosts flags.token flags.apiBaseUrl GotPosts
            else
                Cmd.none
    in
    ( { page = initialPage
...
update msg model =
    case msg of
        Navigate page ->
            let
                cmd =
                    case page of
                        Dashboard ->
                            Api.getPosts model.token model.apiBaseUrl GotPosts

                        Accounts ->
                            Api.getAccounts model.token model.apiBaseUrl GotAccounts

                        Calendar ->
                            Api.getCalendar model.token model.apiBaseUrl GotCalendar
        ...
        DoLogin ->
            ( { model | loading = True }
            , Api.loginRequest model.loginEmail model.loginPassword model.apiBaseUrl LoginResult
            )
        ...
        LoginResult result ->
            case result of
                Ok token ->
                    ...
                    , Cmd.batch
                        [ saveToken token
                        , Api.getPosts (Just token) model.apiBaseUrl GotPosts
                        ]
                    )
        ...
        DoRegister ->
            ( { model | loading = True }
            , Api.registerRequest model.registerEmail model.registerPassword model.apiBaseUrl RegisterResult
            )
            ...
            let
                ...
                cmd =
                    case compose.aiPrompt of
                        Just prompt ->
                            Api.generateContent model.token prompt compose.platform model.apiBaseUrl GotGeneratedContent
                        Nothing ->
                            Cmd.none
            in
            ...
            let
                ...
                cmd =
                    case compose.selectedAccount of
                        Just accountId ->
                            Api.createPost model.token accountId compose.content compose.platform compose.scheduledAt model.apiBaseUrl PostCreated
                        Nothing ->
                            Cmd.none
            in
            ( model, cmd )
        ...
        PublishPost postId ->
            ...
            , Api.publishPost model.token postId model.apiBaseUrl PostPublished
            )

        PostPublished result ->
            let
                newModel = { model | publishing = Nothing }
            in
            case result of
                Ok _ ->
        ...
        DeletePost postId ->
            ( model, Api.deletePost model.token postId model.apiBaseUrl PostDeleted )
        ...
        ConnectAccount ->
            ( model, Api.connectAccount model.token model.apiBaseUrl GotOAuthUrl )
        ...
        DeleteAccount accountId ->
            ( model, Api.deleteAccount model.token accountId model.apiBaseUrl AccountDeleted )
    ...
    div [ class "page login-page" ]
        [ div [ class "login-card" ]
            ...
            , div [ class "login-form" ]
                [ h2 [] [ text "Login" ]
                , input [ placeholder "Email", type_ "email", value model.loginEmail, onInput LoginEmail ] []
                , input [ placeholder "Password", type_ "password", value model.loginPassword, onInput LoginPassword, onEnter DoLogin ] []
                , button [ onClick DoLogin, disabled model.loading, class "btn-primary btn-full" ]
                    [ text (if model.loading then "Logging in..." else "Login") ]
                , div [ class "divider" ] [ text "or" ]
                , h2 [] [ text "Register" ]
                , input [ placeholder "Email", type_ "email", value model.registerEmail, onInput RegisterEmail ] []
                , input [ placeholder "Password (min 8 characters)", type_ "password", value model.registerPassword, onInput RegisterPassword, onEnter DoRegister ] []
                , button [ onClick DoRegister, disabled model.loading, class "btn-secondary btn-full" ]
    ...
    let
        filteredPosts =
            case model.dashboardFilter of
                All -> model.posts
                Drafts -> List.filter (\p -> p.status == "draft") model.posts
                Scheduled -> List.filter (\p -> p.status == "scheduled") model.posts
                Published -> List.filter (\p -> p.status == "published") model.posts
                Failed -> List.filter (\p -> p.status == "failed") model.posts
    ...
    div [ class "page" ]
        ...
        , if List.isEmpty filteredPosts then
            div [ class "empty-state" ]
                [ text (if List.isEmpty model.posts then "No posts yet." else "No posts matching this filter.") ]
          else
            div [ class "posts-list" ] (List.map (viewPostCard model) filteredPosts)
        ]
...
filterTab filter label count currentFilter =
    button
        [ class (if filter == currentFilter then "filter-tab active" else "filter-tab")
...
viewPostCard model post =
    let
        isPublishing = model.publishing == Just post.id
        isDeleting = model.deleteConfirm == Just post.id
    in
        ...
        [ div [ class "post-header" ]
            ...
            , div [ class "post-actions-top" ]
                [ if post.status == "draft" || post.status == "scheduled" then
                    button [ onClick (PublishPost post.id), disabled isPublishing, class "btn-primary btn-sm" ]
                        [ text (if isPublishing then "Publishing..." else "Publish") ]
                  else
                    text ""
                , button [ onClick (ShowDeleteConfirm post.id), class "btn-ghost btn-sm" ] [ text "Delete" ]
                ]
            ]
        , div [ class "post-content" ] [ text post.content ]
        , case post.mediaUrl of
            Just url -> div [ class "post-media" ] [ img [ src url, alt "Post media" ] [] ]
            Nothing -> text ""
        , div [ class "post-meta" ]
            [ case post.scheduledAt of
                Just scheduled -> span [] [ text ("Scheduled: " ++ formatDateTime scheduled) ]
                Nothing -> text ""
            , case post.publishedAt of
                Just published -> span [] [ text ("Published: " ++ formatDateTime published) ]
                Nothing -> text ""
            ]
        , if isDeleting then
...
viewAccounts model =
    div [ class "page" ]
        ...
        , if List.isEmpty model.accounts then
            ...
            div [ class "accounts-grid" ] (List.map viewAccountCard model.accounts)
        ]
    ...
    div [ class "account-card" ]
        ...
        , div [ class "account-meta" ]
            [ case account.tokenExpiresAt of
                Just expires -> div [] [ text ("Token expires: " ++ formatDateTime expires) ]
                Nothing -> text ""
            ]
        , button [ class "btn-ghost btn-sm", onClick (DeleteAccount account.id) ] [ text "Disconnect" ]
        ]
...
viewComposer model =
    let
        compose = model.compose
        canSubmit = compose.selectedAccount /= Nothing && not (String.isEmpty compose.content)
    in
    div [ class "page" ]
        [ div [ class "page-header" ]
        ...
        , div [ class "composer-layout" ]
            [ div [ class "composer-main" ]
                    ...
                    div [ class "form-group" ]
                        ...
                        , select [ onInput UpdateComposeAccount ]
                            (option [ value "" ] [ text "Select account..." ]
                                :: List.map (\a -> option [ value a.id ] [ text (a.username ++ " (" ++ a.provider ++ ")") ]) model.accounts
                            )
                        ]
                , div [ class "form-row" ]
                    [ div [ class "form-group" ]
                ...
                , div [ class "form-group" ]
                    [ label [] [ text "Content" ]
                    , textarea [ placeholder "What's on your mind?", value compose.content, onInput UpdateComposeContent, class "composer-textarea" ] []
                    , div [ class "char-count" ] [ text (String.fromInt (String.length compose.content) ++ "/500") ]
                    ]
                , case compose.mediaType of
                    "IMAGE" -> div [ class "form-group" ] [ label [] [ text "Image URL" ], input [ placeholder "https://example.com/image.jpg", onInput UpdateComposeMediaUrl, value (compose.mediaUrl |> Maybe.withDefault "") ] [] ]
                    "VIDEO" -> div [ class "form-group" ] [ label [] [ text "Video URL" ], input [ placeholder "https://example.com/video.mp4", onInput UpdateComposeMediaUrl, value (compose.mediaUrl |> Maybe.withDefault "") ] [] ]
                    _ -> text ""
                , div [ class "form-group" ]
                    [ label [] [ text "Schedule (optional)" ]
                    , input [ type_ "datetime-local", onInput (\v -> UpdateComposeSchedule (if String.isEmpty v then Nothing else Just v)) ] []
                    ]
                , div [ class "composer-actions" ]
                    [ button [ onClick CreatePost, disabled (not canSubmit), class "btn-primary" ] [ text "Save Draft" ]
                    ]
                ]
            , div [ class "composer-sidebar" ]
                [ div [ class "ai-section" ]
                    ...
                    , textarea [ placeholder "e.g. Write a post about our new product launch...", onInput UpdateAiPrompt, value (compose.aiPrompt |> Maybe.withDefault ""), class "ai-input" ] []
                    , button [ onClick GenerateContent, disabled compose.aiGenerating, class "btn-secondary btn-full" ]
                        [ text (if compose.aiGenerating then "Generating..." else "Generate with AI") ]
                    ]
                , div [ class "preview-section" ]
                    [ h3 [] [ text "Preview" ]
                    , div [ class "post-preview" ]
                        [ div [ class "preview-content" ] [ text (if String.isEmpty compose.content then "Your post content will appear here..." else compose.content) ]
                        , div [ class "preview-meta" ] [ text compose.platform ]
                        ]
                    ]
                ]
            ]
...
viewCalendar model =
    div [ class "page" ]
        ...
        , if List.isEmpty model.calendar then
            ...
            div [ class "calendar-grid" ] (List.map viewCalendarDay model.calendar)
        ]
...
viewCalendarDay day =
    div [ class "calendar-day" ]
        ...
        , div [ class "calendar-posts" ] (List.map viewCalendarPost day.posts)
        ]
...
viewCalendarPost post =
    div [ class ("calendar-post status-" ++ post.status) ]
        [ div [ class "calendar-post-time" ]
            [ text (formatDateTime (Maybe.withDefault "" post.scheduledAt)) ]
        , div [ class "calendar-post-content" ] [ text (String.left 50 post.content) ]
        , div [ class "calendar-post-meta" ] [ span [ class "platform-badge" ] [ text post.platform ] ]
        ]
...
viewAnalytics =
    div [ class "page" ]
        [ h1 [] [ text "Analytics" ]
        , div [ class "empty-state" ] [ text "Analytics coming soon. Connect an account to see insights." ]
        ]
...
viewSettings =
    div [ class "page" ]
        [ h1 [] [ text "Settings" ]
        , div [ class "empty-state" ] [ text "Settings coming soon." ]
        ]
